...
firebase.firestore().enablePersistence().catch(() => { }); // best-effort offline
...
function iconByRating(r) {
  ...
  return L.icon({ iconUrl: 'data:image/svg+xml;base64,' + btoa(svg), iconSize: [24, 24], iconAnchor: [12, 12], popupAnchor: [0, -10] });
}
...
async function deleteReview(id) {
  ...
  try {
    ...
    console.error(e); alert('Delete failed. Please try again.');
  }
}

function editFormHTML(id, data) {
  const ratingUpChecked = data.rating !== 'down' ? 'checked' : '';
  const ratingDownChecked = data.rating === 'down' ? 'checked' : '';
  const hasTag = t => Array.isArray(data.tags) && data.tags.includes(t) ? 'checked' : '';

  return `
    <div style="font:13px/1.35 ui-sans-serif,system-ui;min-width:240px">
      <div style="font-weight:700;margin-bottom:6px">Edit review</div>

      <label style="display:block;margin:6px 0">Address
        <input id="e-addr-${id}" type="text" value="${(data.address || '').replace(/"/g, '&quot;')}" style="width:100%;padding:6px;border:1px solid #394150;background:#0f1218;color:#e9eef5;border-radius:8px"/>
      </label>

      <label style="display:block;margin:6px 0">Facility type
        <select id="e-type-${id}" style="width:100%;padding:6px;border:1px solid #394150;background:#0f1218;color:#e9eef5;border-radius:8px">
          ${[
      ...
      'Other'
    ].map(opt => `<option ${data.facilityType === opt ? 'selected' : ''}>${opt}</option>`).join('')}
        </select>
      </label>

      <fieldset style="margin:6px 0;border:1px solid #394150;border-radius:8px;padding:8px">
        <legend style="padding:0 6px;color:#9aa4b2">Rating</legend>
        <label style="margin-right:10px"><input type="radio" name="e-rate-${id}" value="up" ${ratingUpChecked}/> 👍 Works well</label>
        <label><input type="radio" name="e-rate-${id}" value="down" ${ratingDownChecked}/> 👎 Hard to use</label>
      </fieldset>

      <div style="margin:6px 0">Tags</div>
      <div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:6px">
        ${[
      'Clear audio cues', 'Strong haptic feedback', 'Button reachable',
      'Confusing intersection', 'High traffic noise', 'Blocked by construction'
    ].map(t => `
          <label style="border:1px solid #394150;border-radius:999px;padding:4px 8px;display:inline-flex;gap:6px;background:#0f1218">
            <input type="checkbox" value="${t}" ${hasTag(t)}/> ${t}
          </label>`).join('')}
      </div>

      <label style="display:block;margin:6px 0">Notes
        <textarea id="e-note-${id}" rows="3" style="width:100%;padding:6px;border:1px solid #394150;background:#0f1218;color:#e9eef5;border-radius:8px">${data.comment || ''}</textarea>
      </label>

      <div style="display:flex;gap:8px;justify-content:flex-end;margin-top:8px">
        <button id="e-cancel-${id}" style="border:1px solid #444;background:#1b1f2a;color:#fff;border-radius:8px;padding:6px 10px;cursor:pointer">Cancel</button>
        <button id="e-save-${id}" style="border:1px solid #3a7cff;background:#3a7cff;color:#0b0c0f;border-radius:8px;padding:6px 10px;cursor:pointer">Save</button>
      </div>
    </div>`;
}

function bindEditHandlers(marker, id, data) {
  const btnSave = document.getElementById(`e-save-${id}`);
  const btnCancel = document.getElementById(`e-cancel-${id}`);
  if (!btnSave || !btnCancel) return;

  btnCancel.onclick = () => { marker.closePopup(); marker.openPopup(); }; // reopen to show view mode

  btnSave.onclick = async () => {
    const addr = document.getElementById(`e-addr-${id}`).value.trim();
    const type = document.getElementById(`e-type-${id}`).value;
    const rating = [...document.querySelectorAll(`input[name="e-rate-${id}"]`)].find(i => i.checked).value;
    const tags = [...marker.getPopup().getElement().querySelectorAll('input[type="checkbox"]')].filter(i => i.checked).map(i => i.value);
    ...
    try {
      ...
      marker.closePopup(); // will rerender on snapshot update
    } catch (e) {
      console.error(e);
...
function renderReview(id, data) {
  const canEdit = currentUser && data.userId === currentUser.uid;
  ...
  const viewHTML = `
    <div style="font:13px/1.35 ui-sans-serif,system-ui;min-width:240px">
      ...
      <div class="muted small" style="margin-top:8px">${new Date(data.updatedAt?.toMillis?.() || data.createdAt?.toMillis?.() || Date.now()).toLocaleString()}</div>
      ${canEdit ? `
        <div style="margin-top:8px;display:flex;gap:8px">
          <button id="edit-${id}" style="border:1px solid #3a7cff;background:#3a7cff;color:#0b0c0f;border-radius:8px;padding:6px 10px;cursor:pointer">Edit</button>
          <button id="del-${id}"  style="border:1px solid #444;background:#1b1f2a;color:#fff;border-radius:8px;padding:6px 10px;cursor:pointer">Delete</button>
        </div>` : ""}
    </div>`;

  let m = reviewMarkers.get(id);
  if (m) {
    m.setLatLng(latlng).setIcon(iconByRating(data.rating)).setPopupContent(viewHTML);
  } else {
    m = L.marker(latlng, { icon: iconByRating(data.rating) }).bindPopup(viewHTML).addTo(reviewsLayer);
  ...
  m.on('popupopen', () => {
    ...
    if (delBtn) delBtn.onclick = () => deleteReview(id);

    const editBtn = document.getElementById(`edit-${id}`);
    if (editBtn) {
      editBtn.onclick = () => {
        m.setPopupContent(editFormHTML(id, data));
        // wait for DOM to mount then wire handlers
        setTimeout(() => bindEditHandlers(m, id, data), 0);
      };
    }
  });
...
if (submitBtn) {
  submitBtn.addEventListener('click', async () => {
    ...
    const doc = {
      ...
      address, facilityType, rating, tags, comment,
