/***** Firebase init (your config included) *****/
...
function renderReview(id, data){
  ...
  countEl.textContent = String(reviewMarkers.size);
}
...
/***** Sidebar interactions (form integrated) *****/
...
const showToast = (msg)=>{ toastEl.textContent = msg; toastEl.style.opacity = 1; setTimeout(()=>toastEl.style.opacity=0, 1200); };

useMapClickBtn.addEventListener('click', ()=>{
  captureNextClick = true;
  chosenLatLngEl.textContent = 'Click a location on the map…';
});
map.on('click', e=>{
  if (!captureNextClick) return;
  captureNextClick = false;
  pendingLatLng = e.latlng;
  chosenLatLngEl.textContent = `Chosen: ${pendingLatLng.lat.toFixed(5)}, ${pendingLatLng.lng.toFixed(5)}`;
});
